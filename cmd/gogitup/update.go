...
func init() {
	...
	updateCmd.Flags().StringP("config", "c", "~/.gogitup.yaml", "config file")
	updateCmd.Flags().StringP("repos-file", "r", "", "repositories file")
	updateCmd.Flags().IntP("threads", "t", 4, "number of concurrent updates")
...
Use the -s or --stat flag to show git diff statistics for updated repositories.`,
	...
	PreRun: func(cmd *cobra.Command, args []string) {
		...
		if err := viper.BindPFlag("threads", cmd.Flags().Lookup("threads")); err != nil {
			fmt.Fprintf(os.Stderr, "Warning: failed to bind threads flag: %v\n", err)
		}
	},
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Create work pool
		numWorkers := viper.GetInt("threads")
		if numWorkers < 1 {
			numWorkers = 1
		}
		...
		if len(errors) > 0 {
			...
			// Return a simple error without details since they were already printed
